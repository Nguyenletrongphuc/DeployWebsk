...
export default function MyTicketsPage() {
  ...
  const [regs, setRegs] = useState([]);    // sự kiện miễn phí
  const [bookings, setBookings] = useState([]); // sự kiện có phí
          ...
          <div className="reg-grid">
              ...
              <div key={r.id} className="reg-card">
                ...
                {r.status === 'CONFIRMED' && (
                  <button className="btn-sm btn-danger" style={{ marginTop: '0.75rem' }}
                    onClick={() => handleCancelReg(r.id)}>Hủy đăng ký</button>
                )}
              </div>
            ))}
            ...
            {visible.bookings.map(b => (
              <div key={b.id} className="reg-card" style={{ borderColor: 'rgba(167,139,250,0.2)' }}>
                ...
                {b.status === 'CONFIRMED' && (
                  <button className="btn-sm btn-danger" style={{ marginTop: '0.75rem' }}
                    onClick={() => handleCancelBooking(b.id)}>Hủy & Hoàn tiền</button>
                )}
              </div>
            ))}
          </div>
